...
uint mod(const T a, const uint m = MOD)
{
    assert(m > 0);
    if(a >= 0)
 ...
 * Space Complexity: O(1).
 */
template<typename T1, typename T2, typename enable_if<is_integral<T1>::value and is_integral<T2>::value, uint>::type = 0>
uint modadd(const T1 a, const T2 b, const uint m = MOD)
{
    assert(m > 0);
 ...
 */
template<typename T1, typename T2, typename enable_if<is_integral<T1>::value and is_integral<T2>::value, uint>::type = 0>
uint modsub(const T1 a, const T2 b, const uint m = MOD)
{
    assert(m > 0);
    uint mod_a = mod(a, m), mod_b = mod(b, m);
    if(mod_b > mod_a)
 ...
 */
template<typename T1, typename T2, typename enable_if<is_integral<T1>::value and is_integral<T2>::value, uint>::type = 0>
uint modmul(const T1 a, const T2 b, const uint m = MOD)
{
    assert(m > 0);
    uint mod_a = mod(a, m), mod_b = mod(b, m);
...
template<typename T,
    typename enable_if<is_integral<T>::value, uint>::type = 0>
uint modinv(const T a, const uint m = MOD)
{
    assert(m > 0);
 ...
 */
template<typename T1, typename T2, typename enable_if<is_integral<T1>::value and is_integral<T2>::value, uint>::type = 0>
uint moddiv(const T1 a, const T2 b, const uint m = MOD)
{
    assert(m > 0);
...
template<typename T,
    typename enable_if<is_integral<T>::value, uint>::type = 0>
uint modexp(const T b, uint e, const uint m = MOD)
{
    assert(m > 0);
